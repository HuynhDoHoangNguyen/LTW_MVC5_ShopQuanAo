...
BEGIN
    ...
    -- Cập nhật thành tiền chi tiết phiếu nhập
    UPDATE CT
    ...
    -- Cập nhật tồn kho từng biến thể SP
    UPDATE S
    ...
    -- Cập nhật tổng số lượng + tiền Phiếu nhập
    UPDATE P
    ...
    -- Cập nhật tổng tồn kho bảng SAN_PHAM
    UPDATE SP
...
BEGIN
    ...
    -- Set giá lúc bán nếu chưa có
    UPDATE CT
    SET CT.GIALUCBAN = S.GIABAN * 1.3
    FROM CT_DON_HANG CT
    JOIN inserted I ON CT.MADH = I.MADH
        AND CT.MASP = I.MASP AND CT.MAMAU = I.MAMAU AND CT.MASIZE = I.MASIZE
    JOIN CHI_TIET_SP S ON S.MASP = I.MASP
        AND S.MAMAU = I.MAMAU AND S.MASIZE = I.MASIZE
    WHERE CT.GIALUCBAN IS NULL;

    -- Cập nhật thành tiền CT đơn hàng
    UPDATE CT
    ...
    -- Chặn âm tồn kho
    IF EXISTS (SELECT 1 FROM CHI_TIET_SP WHERE SOLUONGTON < 0)
    BEGIN
    ...
    -- Cập nhật tổng số lượng + giảm giá + tổng tiền đơn
    UPDATE D
    SET D.TONGSOLUONG = (SELECT SUM(SOLUONG) FROM CT_DON_HANG WHERE MADH = D.MADH),
        D.GIAMGIA = CASE 
